...
import { ArrowForwardFilled, FilterAltOutlined, FilterAltTwotone } from '@vicons/material'
    ...
    methods: {
        ...
        async onMapLoaded() {
            ...
            if (this.$route.query.start) {
                const routeStartDate = moment(this.$route.query.start, 'YYYY-MM-DD').startOf('day');
                if (routeStartDate.isValid() && !this.dateIsInvalid(routeStartDate)) {
                    start = routeStartDate.unix() * 1000;
                }
            }

            let end = this.dataEndDate;
            if (this.$route.query.end) {
                const routeEndDate = moment(this.$route.query.end, 'YYYY-MM-DD').startOf('day');
                if (routeEndDate.isValid() && !this.dateIsInvalid(routeEndDate)) {
                    end = routeEndDate.unix() * 1000;
                }
            }

            if (_.has(this.$route.query, 'injury')) {
                this.injuryOnly = true;
            }

            if (_.has(this.$route.query, 'barrels')) {
                this.showBarrels = true;
            }

            if (this.$route.query.casings) {
                this.minCasings = parseInt(_.get(this.$route.query, 'casings', '0'));
            }
