        ...
        <n-collapse v-if="!loading">
            <n-collapse-item v-for="item in sortedUniqueItems" :key="item.id">
                ...
                <n-table class="incident_table">
                    ...
                    <tr>
                        <th colspan="2">
                            <a target="_blank" :href="getSearchUrl(item)">Search PortlandOregon.gov</a>
                        </th>
                    </tr>
                    </tbody>
                </n-table>
...
export default {
    ...
    methods: {
        ...
        getSearchUrl(item) {
            const date = _.toLower(moment.utc(item.date).format('MMMM+Do+YYYY'));
            return `//duckduckgo.com/?q=shooting+${date}+${item.id}+portland+or+site:portlandoregon.gov`;
        },
    },
    components: {
        NStatistic,
